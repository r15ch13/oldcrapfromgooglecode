...
/**
 * Gibt true zurück wenn der Wert zwischen den beiden anderen Werten liegt.
 *
 * @author Richard 'r15ch13' Kuhnt
 * @param numeric Wert der geprüft wird
 * @param numeric kleiner Wert
 * @param numeric großer Wert
 * @return boolean
 * @access public
 */
...
/**
 * Prüft, ob der Wert ein gültiger {@link http://de.wikipedia.org/wiki/Sha1 SHA1-Hash} ist
 *
 * @param string SHA1-Hash
 * @return string Gibt <b>true</b> zurück, wenn der Wert ein SHA1-Hash ist.
 */
 ...
 * @param string Salt
 * @return string Gibt SHA1-Hash zurück;
 */
...
function pharseXml($text) {
    ...
    $text = str_replace("Þ", "&THORN;", $text);
    $text = str_replace("Ð", "&ETH;", $text);
    $text = str_replace("þ", "&thorn;", $text);
    $text = str_replace("ð", "&eth;", $text);
    $text = str_replace("Ø", "&Oslash; ", $text);
    $text = str_replace("Œ", "&OElig;", $text);
    $text = str_replace("œ", "&oelig;", $text);
    $text = str_replace("Š", "&Scaron;", $text);
    $text = str_replace("š", "&scaron;", $text);
    $text = str_replace("<", "&lt;", $text);
    $text = str_replace(">", "&gt;", $text);
    return trim($text);
